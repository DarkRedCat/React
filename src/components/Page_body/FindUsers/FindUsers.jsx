        ...
        <div key={u.id - 1} className={classes.wrapp}>
          <div className={classes.left}>
            <div className={classes.avatar}>
              <NavLink to={`/profile/id_${u.id}`} onClick={() => {props.state.gg(u.id)}}>{u.photos.small 
                  ? <img src={`${u.photos.small}`} alt="1"/>:<img src={`https://picsum.photos/80/80?random=${u.id}`} alt="1" />}
              </NavLink>
            </div>
        
            <div className={classes.button}>{(u.followed)
              ? <button disabled={props.state.followingInProgress.some(id => id === u.id)} 
                  onClick={() => {props.state.unFollow(u.id)}}>{props.state.followingInProgress.some(id => id === u.id)
                    ? <div className={classes.BL}><Load className={classes.load}/> </div>: 'unfolowed'}
                </button>
              : <button disabled={props.state.followingInProgress.some(id => id === u.id)} 
                  onClick={() => {props.state.follow(u.id)}}>{props.state.followingInProgress.some(id => id === u.id) 
